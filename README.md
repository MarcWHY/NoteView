...
确认后可在 OBS 画面中把来源缩小到需要的尺寸；1.5 倍像素输出会让谱线、音符、琴键和文字在缩放后更清晰。最小化 NoteView 后 MIDI 接收与独立输出会继续运行；退出程序会停止输出。背景跟随 NoteView 的背景颜色和不透明度，设置为全透明即可叠加在 OBS 其他来源上。地址只在本机使用，不需要互联网或额外插件。完整设置、黑屏排查见 [OBS 使用说明](docs/OBS.md)。

## 显示与和弦识别

v0.2.1 优化了实时延迟：MIDI 消息合并后立即刷新，OBS 使用最新快照和有界长轮询；静态琴键、谱线、布局及图像缓冲区复用，PNG 使用快速无损编码。升级后在 OBS 浏览器源属性中点击一次 **刷新当前页面缓存**，使已有来源加载新的取帧逻辑。分辨率仍为 1680×960，和声色彩与踏板虚化保留。实测和复现命令见 [性能说明](docs/PERFORMANCE.md)。

- 默认完整 88 键 A0–C8；也可选择 C2–C6。常用音域模式下，范围外音符仍可参与和弦识别，但不显示在谱面上。
- 五线谱、键盘、和弦是统一矩形画布中的三个独立控件，移动或缩放一个不会改变另外两个；尺寸和位置受画布边界约束。桌面随窗口大小整体适配，OBS 保持 1120×640 的逻辑布局并以 1680×960 像素输出。C1–C8 八度文字已移除。
