...
$testOutput = Join-Path $projectRoot 'artifacts\obs-integration-tests'
...
Get-Content -LiteralPath (Join-Path $testOutput 'test.log')
if ($testProcess.ExitCode -ne 0) { Get-Content -LiteralPath (Join-Path $testOutput 'errors.log'); throw 'OBS integration tests failed.' }
