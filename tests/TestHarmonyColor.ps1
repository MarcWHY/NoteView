...
$testOutput = Join-Path $projectRoot 'artifacts\harmony-color-tests'
...
$testExe = Join-Path $testOutput 'HarmonyColorTests.exe'
& (Join-Path $framework 'csc.exe') /nologo /target:exe /platform:x64 /optimize+ /unsafe+ /codepage:65001 /main:NoteView.HarmonyColorTests ('/out:' + $testExe) @references @sources (Join-Path $PSScriptRoot 'HarmonyColorTests.cs')
