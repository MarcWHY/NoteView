...
$testOutput = Join-Path $projectRoot 'artifacts\harmonic-context-tests'
...
$testExe = Join-Path $testOutput 'HarmonicContextTests.exe'
& (Join-Path $framework 'csc.exe') /nologo /target:exe /platform:x64 /optimize+ /unsafe+ /codepage:65001 /main:NoteView.HarmonicContextTests ('/out:' + $testExe) @references @sources (Join-Path $PSScriptRoot 'HarmonicContextTests.cs')
