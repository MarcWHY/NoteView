...
$sources = @('MusicTheory.cs','KeySignature.cs','StaffView.cs', 'LayoutBoard.cs','AppSettings.cs','HarmonyVisuals.cs','ObsFrameRenderer.cs','ObsFrameWorker.cs') | ForEach-Object { Join-Path $projectRoot ('src\' + $_) }
$sources += Join-Path $PSScriptRoot 'ObsFrameWorkerTests.cs'
$testExe = Join-Path $testOutput 'ObsFrameWorkerTests.exe'
& (Join-Path $framework 'csc.exe') /nologo /target:exe /platform:x64 /codepage:65001 /main:NoteView.ObsFrameWorkerTests ('/out:' + $testExe) @references @sources
