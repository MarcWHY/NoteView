...
$testOutput = Join-Path $projectRoot 'artifacts\harmony-history-tests'
...
Get-Content -LiteralPath (Join-Path $testOutput 'history-test.log')
if ($testProcess.ExitCode -ne 0) { Get-Content -LiteralPath (Join-Path $testOutput 'history-test-errors.log'); throw 'Harmony history tests failed.' }
