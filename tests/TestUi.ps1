...
& (Join-Path $framework 'csc.exe') /nologo /target:exe /platform:x64 /optimize+ /unsafe+ /codepage:65001 /main:NoteView.UiSmokeTests ('/out:' + $testExe) @references @sources (Join-Path $PSScriptRoot 'UiSmokeTests.cs')
...
$testProcess = Start-Process -FilePath $testExe -WindowStyle Hidden -PassThru -Wait -RedirectStandardOutput (Join-Path $testOutput 'ui-test.log') -RedirectStandardError (Join-Path $testOutput 'ui-test-errors.log')
Get-Content -LiteralPath (Join-Path $testOutput 'ui-test.log')
if ($testProcess.ExitCode -ne 0) { Get-Content -LiteralPath (Join-Path $testOutput 'ui-test-errors.log'); throw 'UI tests failed.' }
